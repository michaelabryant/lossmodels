...
def test_mean_from_discretized_pmf_close_for_exponential():
    sev = Exponential(rate=1.0)
    pmf = discretize_severity(sev, h=0.1, max_loss=10.0)

    approx_mean = mean_from_discretized_pmf(pmf, h=0.1)
    assert np.isclose(approx_mean, sev.mean(), rtol=0.1)


def test_mean_from_discretized_pmf_close_for_lognormal():
    sev = Lognormal(mu=1.0, sigma=0.5)
    pmf = discretize_severity(sev, h=0.25, max_loss=20.0)

    approx_mean = mean_from_discretized_pmf(pmf, h=0.25)
    assert np.isclose(approx_mean, sev.mean(), rtol=0.15)
