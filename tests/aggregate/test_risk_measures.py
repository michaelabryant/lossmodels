...
from lossmodels.aggregate import var, tvar, stop_loss, lev, exceedance_probability
...
def test_var_basic():
    ...
    expected = np.quantile(losses, 0.5)
...
def test_tvar_returns_var_if_no_tail_values():
    losses = np.array([5.0, 5.0, 5.0, 5.0])

    result = tvar(losses, 0.95)
    expected = np.quantile(losses, 0.95)
