...
def test_policy_limit_cdf_matches_formula():
    sev = Exponential(rate=1.5)
    model = PolicyLimit(severity=sev, u=2.0)

    assert model.cdf(-0.1) == 0.0
    assert np.isclose(model.cdf(0.5), sev.cdf(0.5))
    assert np.isclose(model.cdf(1.99), sev.cdf(1.99))
    assert model.cdf(2.0) == 1.0
...
def test_policy_limit_can_be_discretized():
    sev = PolicyLimit(Exponential(rate=1.0), u=2.0)
